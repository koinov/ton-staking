...
td::Ref<vm::Stack> prepare_vm_stack(td::Ref<vm::CellSlice> body, td::int64 grams, block::StdAddress address ) {
  td::Ref<vm::Stack> stack_ref{true};
  td::RefInt256 acc_addr{true};
  CHECK(acc_addr.write().import_bits(address.addr.cbits(), 256));
  vm::Stack& stack = stack_ref.write();
  stack.push_int(td::RefInt256{true, 10000000000});
  stack.push_int(td::RefInt256{true, 10000000000});
  stack.push_int(td::RefInt256{true, grams});
  stack.push_cell(vm::CellBuilder().store_long(0, 4).store_long(4, 3).store_long(address.workchain,8).store_bits(address.addr.bits(),256).finalize());
  stack.push_cellslice(std::move(body));
  return stack_ref;
}
...
StakingSmartContract::Answer run_smartcont(StakingSmartContract::State state, td::Ref<vm::Stack> stack, td::Ref<vm::Tuple> c7,
  ...
  StakingSmartContract::Answer res;
...
td::Ref<vm::CellSlice> StakingSmartContract::empty_slice() {
  return vm::load_cell_slice_ref(vm::CellBuilder().finalize());
}

size_t StakingSmartContract::code_size() const {
  return vm::std_boc_serialize(state_.code).ok().size();
}
size_t StakingSmartContract::data_size() const {
  return vm::std_boc_serialize(state_.data).ok().size();
}

block::StdAddress StakingSmartContract::get_address(WorkchainId workchain_id) const {
  return GenericAccount::get_address(workchain_id, get_init_state());
}

td::Ref<vm::Cell> StakingSmartContract::get_init_state() const {
  return GenericAccount::get_init_state(get_state().code, get_state().data);
}

StakingSmartContract::Answer StakingSmartContract::run_method(Args args ) {
  if (!args.c7) {
...
StakingSmartContract::Answer StakingSmartContract::run_get_method(Args args) const {
  if (!args.c7) {
    args.c7 = prepare_vm_c7();
  }
  if (!args.limits) {
...
StakingSmartContract::Answer StakingSmartContract::run_get_method(td::Slice method, Args args) const {
  return run_get_method(args.set_method_id(method));
}


StakingSmartContract::Answer StakingSmartContract::send_external_message(td::Ref<vm::Cell> cell, Args args)  {
  return run_method(args.set_stack(prepare_vm_stack(vm::load_cell_slice_ref(cell))).set_method_id(-1));
}

StakingSmartContract::Answer StakingSmartContract::test_send_internal_message(td::Ref<vm::Cell> cell, td::int64 grams, block::StdAddress address_from, Args args)  {
  return run_method( args.set_stack(prepare_vm_stack(vm::load_cell_slice_ref(cell), grams, address_from )).set_method_id(0).set_limits(vm::GasLimits{(long long)0, (long long)1000000, (long long)100000}) );
}
