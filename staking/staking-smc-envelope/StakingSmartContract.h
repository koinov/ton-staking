...
namespace ton {
class StakingSmartContract : public td::CntObject {
 ...
 public:
  ...
  StakingSmartContract(State state) : state_(std::move(state)) {
  }

  struct Answer {
    StakingSmartContract::State new_state;
    bool accepted;
    bool success;
  ...
  Answer test_send_internal_message(td::Ref<vm::Cell> cell, td::int64 grams, block::StdAddress address_from, Args args = {});

  size_t code_size() const;
  size_t data_size() const;
  static td::Ref<StakingSmartContract> create(State state) {
    return td::Ref<StakingSmartContract>{true, std::move(state)};
  }
