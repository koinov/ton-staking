...
void init_wallet_v3(Client& client, std::string pkey, std::string password){
  auto secret_key = td::Ed25519::PrivateKey::from_pem(pkey, password).move_as_ok();
  auto public_key = secret_key.get_public_key().move_as_ok();


  ton::WalletV3 w( {ton::WalletV3::get_init_code(),  ton::WalletV3::get_init_data( public_key, default_wallet_id )});

  auto init_message = w.get_init_message( secret_key, default_wallet_id );
  auto address = w.get_address(0).rserialize();

  auto ans = w.send_external_message(init_message);
  LOG(INFO) << "TEST WALLET CHECK CODE " << address << " ans.code : " << ans.code;

  auto query_id = create_raw_query( client, address,
      vm::std_boc_serialize(w.get_init_code()).move_as_ok().as_slice().str(),
      vm::std_boc_serialize(w.get_init_data(  public_key, default_wallet_id )).move_as_ok().as_slice().str(),
      vm::std_boc_serialize(w.get_init_message(secret_key, default_wallet_id  )).move_as_ok().as_slice().str()
  ).move_as_ok();
  LOG(INFO) << "INIT WALLET V3 " << address << " " << query_id.id;

  auto a_state = get_account_state(client, address);

  LOG(INFO) << "Account state  lt :" << a_state.last_transaction_id.lt << " balance:" << a_state.balance;
  if( a_state.type != AccountState::Empty){
    query_send( client, query_id);
    auto new_a_state = wait_state_change(client, a_state, a_state.sync_utime + 30).move_as_ok();
  }
}

void init_pool(Client& client,  std::string pkey, std::string password, td::uint32 nominators_count){
    auto secret_key = td::Ed25519::PrivateKey::from_pem(pkey, password).move_as_ok();
    auto public_key = secret_key.get_public_key().move_as_ok();

    ton::WalletV3 master_wallet( {ton::WalletV3::get_init_code(),  ton::WalletV3::get_init_data( public_key, default_wallet_id )});

    Wallet giver_wallet = import_wallet_from_pkey(client, pkey, password);
    LOG(INFO) << "Master address : " << master_wallet.get_address(0).rserialize() << " (" << master_wallet.get_address(0) << ")";


    ton::StakingPool pool({
            ton::StakingPool::get_init_code(),
            ton::StakingPool::get_init_data(master_wallet.get_address(), 500, 1000000)
        });

    auto pool_address = pool.get_address(0);
    LOG(INFO) << "Pool address : " << pool_address.rserialize() << " (" << pool_address << ")";

    std::vector<block::StdAddress> nominator_addresses;

    for( td::uint32 a = 0 ; a < nominators_count; a++ ){
        ton::Nominator nominator({
          ton::Nominator::get_init_code(),
          ton::Nominator::get_init_data(pool_address, a)
        });
        auto nominator_address = nominator.get_address(-1);
        nominator_addresses.push_back(nominator_address);
        LOG(INFO) << "Nominator #" << a << " address : " << nominator_address.rserialize() << " (" << nominator_address << ")";
    }

    auto a_state = get_account_state(client, pool_address.rserialize());

    transfer_grams(client, giver_wallet, pool_address.rserialize(), 1000000000);

    auto new_a_state = wait_state_change(client, a_state, a_state.sync_utime + 30).move_as_ok();


    auto query_id = create_raw_query( client, pool_address.rserialize(),
                                      vm::std_boc_serialize(pool.get_init_code()).move_as_ok().as_slice().str(),
                                      vm::std_boc_serialize(pool.get_init_data(master_wallet.get_address(), 500, 1000000 )).move_as_ok().as_slice().str(),
                                      vm::std_boc_serialize(pool.get_init_message(nominator_addresses)).move_as_ok().as_slice().str()
    ).move_as_ok();
    LOG(INFO) << "INIT POOL " << pool_address << " " << query_id.id;

    a_state = get_account_state(client, pool_address.rserialize());
    if( a_state.type == AccountState::Empty){
      LOG(INFO) << "SENDING INIT POOL MESSAGE";
      query_send( client, query_id);
      auto new_a_state = wait_state_change(client, a_state, a_state.sync_utime + 30).move_as_ok();
    }
...
void pool_info(Client& client,  std::string pkey, std::string password, td::uint32 nominators_count){
  auto secret_key = td::Ed25519::PrivateKey::from_pem(pkey, password).move_as_ok();
  auto public_key = secret_key.get_public_key().move_as_ok();
  ...
  auto pool_account_address = make_object<tonlib_api::accountAddress>(pool_address.rserialize());
  ...
  auto current_period = current_period_result.stack.write().pop_smallint_range(100000);
  LOG(INFO) << "Current period : " << current_period;
  auto data_hash = current_pool.get_data_hash();
  LOG(INFO) << "Data hash : " << td::base64_encode(data_hash.as_slice());
...
int main(int argc, char* argv[]) {
  ...
  Wallet giver_wallet = import_wallet_from_pkey(client, giver_key_str, giver_key_pwd);
  //init_wallet_v3(client, giver_key_str, giver_key_pwd);
  //init_pool(client, giver_key_str, giver_key_pwd, 10);
  pool_info(client, giver_key_str, giver_key_pwd, 10);
// LOG(INFO) << giver_wallet.address;
